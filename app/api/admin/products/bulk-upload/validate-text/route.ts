  ...
  try {
    ...
    if (!rows || !Array.isArray(rows) || rows.length === 0) {
      return NextResponse.json({
        ...
        validRows: 0,
        invalidRows: 0,
        errors: [{ row: 0, column: 'data', message: 'No products provided' }],
        parsedData: []
      }, { status: 400 })
    }
    ...
    if (rows.length > 1000) {
      return NextResponse.json({
        ...
        errors: [{ row: 0, column: 'data', message: 'Maximum 1000 rows allowed per upload' }],
        parsedData: []
      }, { status: 400 })
    }
...
async function validateRow(
  ...
  if (name) {
    if (seenNames.has(name.toLowerCase())) {
      errors.push({ row: rowNum, column: 'name', message: `Duplicate name "${name}" in data` })
    }
  ...
  if (slug) {
    if (seenSlugs.has(slug.toLowerCase())) {
      errors.push({ row: rowNum, column: 'slug', message: `Duplicate slug "${slug}" in data` })
    }
      ...
      for (const label of variantLabels) {
        if (!unitLabels.includes(label)) {
          errors.push({
            row: rowNum,
            column: 'variants',
            message: `Variant "${label}" not found for productType "${productType}". Available: ${unitLabels.join(', ')}`
          })
        }
      }
    }
